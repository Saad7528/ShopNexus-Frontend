            ...
            {[1, 2, 3, 4, 5].map((star) => (
              <Star
                ...
                className={`w-3.5 h-3.5 ${
                  star <= Math.round(averageRating) ? 'fill-amber-400' : 'text-slate-600'
                }`}
              />
            ))}
          </div>
          <p className="text-[11px] text-slate-400 mt-1">{totalReviews} verified reviews</p>
        </div>

        <div className="flex-1 space-y-1.5 border-l border-slate-800 pl-4">
          {[5, 4, 3, 2, 1].map((rating) => {
            ...
            return (
              <div key={rating} className="flex items-center gap-2 text-xs">
                <span className="w-3 font-semibold text-slate-300">{rating}</span>
                ...
                <span className="w-8 text-right text-[11px] text-slate-400">{percentage}%</span>
              </div>
            );
          })}
        </div>
