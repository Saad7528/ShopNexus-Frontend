            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h2 className="text-sm font-bold text-white flex items-center gap-2">
                  Notification Center
                  {unreadCount > 0 && (
                    <span className="text-[10px] px-2 py-0.2 rounded-full bg-rose-500 text-white font-bold animate-pulse">
                      {unreadCount} New
                    </span>
                  )}
                </h2>
                <p className="text-[10px] text-slate-400">Price Drops, Promos & Real-Time Alerts</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1">
              {unreadCount > 0 && (
                <button
                  ...
                  className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-[10px] font-semibold transition-colors cursor-pointer"
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {notifications.length === 0 ? (
              <div className="text-center py-16 space-y-3">
                ...
                <p className="text-[10px] text-slate-500">We will notify you when prices drop on your wishlist!</p>
              </div>
            ) : (
              notifications.map((n) => (
                <div
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {n.oldPrice && n.newPrice && (
                        <div className="flex items-center gap-2 mt-2 font-mono text-xs">
                          <span className="line-through text-slate-500">${n.oldPrice}</span>
                          <span className="font-bold text-emerald-400">${n.newPrice}</span>
                        </div>
                      )}
