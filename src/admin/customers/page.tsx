      ...
      <div className="space-y-8 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-black text-white">Staff Roles & Customer Security</h1>
        ...
        <div className="p-6 rounded-3xl bg-slate-900/60 border border-slate-800 space-y-5">
          <div className="flex items-center justify-between border-b border-slate-800 pb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-black text-white">Staff Accounts & RBAC Permissions</h2>
            </div>
            <span className="text-xs text-slate-400 font-semibold">{staffList.length} Authorized Team Members</span>
          </div>
                ...
                {staffList.map((st) => (
                  <tr key={st.id} className="hover:bg-slate-800/30 transition-colors">
                    <td className="px-5 py-3.5">
                      ...
                      <span className="text-[10px] text-slate-500">Joined {st.createdAt}</span>
                    </td>
                    <td className="px-5 py-3.5 font-mono text-indigo-300 text-[11px]">{st.email}</td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2">
                        <span className="px-2.5 py-1 rounded-full bg-slate-800 text-slate-200 font-bold text-[10px] border border-slate-700">
                          {st.role}
                        </span>
                        ...
                        <button
                          ...
                          className="p-1 rounded-lg bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 text-xs transition-colors cursor-pointer"
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        {st.permissions.canManageCatalog && (
                          <span className="px-2 py-0.5 rounded bg-blue-500/10 text-blue-400 text-[9px] font-bold">
                            Catalog
                          </span>
                        )}
                        ...
                        {st.permissions.canAccessRBAC && (
                          <span className="px-2 py-0.5 rounded bg-purple-500/10 text-purple-400 text-[9px] font-bold">
                            Master Admin
                          </span>
                        )}
                      </div>
        ...
        <div className="p-6 rounded-3xl bg-slate-900/60 border border-slate-800 space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-black text-white">Customer Lifetime Value (LTV) & Fraud Detection</h2>
            </div>
            <span className="text-xs text-slate-400 font-semibold">Track customer return history & block fake spam orders</span>
          </div>
                ...
                {customers.map((cust) => (
                  <tr key={cust.id} className="hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-5 py-3.5 font-mono text-[11px] text-slate-300">
                      <div>{cust.phone}</div>
                      <span className="text-[10px] text-slate-500 font-sans">{cust.email}</span>
                    </td>
                    <td className="px-5 py-3.5 font-semibold text-white">{cust.ordersCount} completed</td>
                    ...
                    <td className="px-5 py-3.5 text-right">
                      ...
                      >
                        {cust.isFlaggedFraud ? '🚫 Blocklisted' : 'Whitelist'}
                      </button>
                    </td>
                  </tr>
                ))}
        ...
        {isAddStaffOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
            <div className="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-4">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <h2 className="text-lg font-black text-white">Create Staff Login</h2>
                <button
                  type="button"
