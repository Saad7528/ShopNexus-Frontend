...
export default function ForgotPasswordPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 sm:p-6 bg-[#0b0f19] text-white">
      <div className="max-w-md w-full p-8 rounded-3xl bg-slate-900/80 border border-slate-800 backdrop-blur-2xl shadow-2xl space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-black tracking-tight text-white">Password Recovery</h1>
        ...
        {statusMessage && (
          <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs space-y-3">
            ...
            {resetToken && (
              <div className="p-3 rounded-xl bg-slate-950/80 border border-emerald-500/30 space-y-2">
                ...
                <code className="text-xs font-mono text-emerald-300 font-bold break-all block selection:bg-emerald-500/30">
                  {resetToken}
                </code>
                <Link
        ...
        {!resetToken && (
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-white text-xs placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-colors"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs shadow-lg shadow-indigo-600/25 transition-all disabled:opacity-50 cursor-pointer"
            >
              {isLoading ? (
                'Processing Request...'
              ) : (
